---
title: 'Fluidigm: An R-package for fluidigm data handling, sexing and clustering'
output:
  pdf_document:
    keep_md: true
tags:
...
nanofluidic dynamic arrays from Standard BioTools Inc., previously known as Fluidigm [@wang2009]. Genotypic data refers here
to the genetic makeup of an organism, encoded in its DNA, which determines its physical and physiological
traits. This data can represent the entire genetic variation, with millions of single-nucleotide
...
It then estimates potential errors, e.g., in form of sequencing errors, from the data, ensuring its accuracy and
...
species markers [@harmoinen2024] or Y and/or X chromosomal markers [@ekblom2021] for diagnosing sex. This requires
...
The `estimateErrors` function is designed to process the generated PLINK ped files and estimate errors. It offers a
comprehensive analysis of genotyping data, ensuring the accuracy and reliability of the results.
...
and the thresholds for various error checks. Further, the function can create a database to store already sequences
samples and use them in subsequent analyses.

In addition, `estimateErrors` can generate plots for visual inspection of the data and provides verbose
output for a detailed function feedback. It returns a list containing a matrix indicating if genotypes are called
...
Optionally, the function can also accept a path to an existing sample database. Or, in case of its absense,
it can also create such a database. If provided, the function will merge the output with this existing
...
a `.pairs`-extension in the same directory as the input file. This output file encapsulates the
...
It conducts a pairwise similarity analysis on the genotypic data, comparing each pair of genotypes in
...
# Availability
The package is available in a stable release version on Cran, see https://cran.r-project.org/web/packages/Fluidigm.
Latest development versions can be found in the `dev` branch of the corresponding GitHub repository
(https://github.com/fischuu/Fluidigm), whereas the `main` branch is always aligned to the latest Cran release.
